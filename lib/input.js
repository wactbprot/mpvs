...
exports.section = function(req, cb){
  var $          = cheerio.load(req.body),
      ...
      $valElems  = $(".value"),
      noOfInp    = $valElems.length,
      co         = {id      : id,
                    key     : key,
                    struct  : "exchange"};
  
  log.info({ok:true, fn:"section"},
           "receive data")

  for(var i = 0; i <  noOfInp; i++){
    var $valElem = $valElems.eq(i),
        subkey   = $valElem.attr("data-subkey"),
        val      = $valElem.attr("value") || "";
                  ...
                  function(j,v){
                    return function(res){
                      ...
                      if(j ===  (noOfInp - 1)){
                        ready(id, key, cb);
                        log.info({ok:true},
                                 "end request, all values written");
                      }
                    }
                  }(i,val));
    ssa.write(val);
...
var ready = function(id, key, cb){
  ...
  var ssa = com(net.set(co), function(res){
              log.info({ok:true},
                       "set key: " + key + " ready for read  out");
              cb("ok")
            });
  ssa.write('true');
  ssa.end()
